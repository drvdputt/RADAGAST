...
cloudy_exe = c17_dir / "source/cloudy.exe"
...
cloudy_template_file = (
    gasmodule_git / "benchmark/cloudy/thin-shell-mrn-template/hsphere.in"
)
...
with open(joblist_file, "w") as jobf, open(gasmod_jobs_file, "w") as gasmodf, open(
    ...
    for nh, tc, lum in itertools.product(densities, color_temperatures, luminosities):
        single_point_output_dir = output_dir / "{:.1e}_{:.1e}_{:.1e}".format(
            nh, tc, lum
        )
        ...
        for f in (jobf, cloudyf):
            f.write(
                "cd {} && {cloudy} -r hsphere\n".format(
                    single_point_output_dir.resolve(), cloudy=cloudy_exe
                )
            )
        ...
        for f in (jobf, gasmodf):
            f.write(
                "cd {} && mkdir -p MRNDust && {gasmodule} {nh} {tc} {lum}\n".format(
                    single_point_output_dir.resolve(),
