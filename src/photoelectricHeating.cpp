...
namespace {
    // New actual constants
    const double _habing = 5.33e-14; // energy density between 6 and 13.6 eV (ergs cm-3)
    const double _e2 = Constant::FINESTRUCTURE * Constant::PLANCKLIGHT / 2 / Constant::PI; // electron charge square in cgs (I think cgs uses Gaussian units or something)
    const double _me = 9.1094e-28; // electron mass in grams
    ...
    const double _gasTemperature = 1.e3;
    
    // Grain propeties to use for tests (more detailes in generateQabs)
    ...
    const double _G0 = 2.45e-0;
    
    // Wavelength grid to use for tests
    const size_t Nlambda = 1000;
    double lambdaMin = 0.0912 * Constant::UM_CM; // cutoff at 13.6 eV
    double lambdaMax = 10 * Constant::UM_CM;
    
    std::vector<double> generateWavelengthGrid()
    {
    ...
    void readQabs() 
    {
        ...
        // open the file
        ifstream file;
        if (_carbonaceous) file.open("/Users/drvdputt/SKIRT/git/dat/GrainComposition/Draine/Gra_81.dat");
        else file.open("/Users/drvdputt/SKIRT/git/dat/GrainComposition/Draine/suvSil_81.dat");
        ...
        for (double& d : _lambdav) 
        {
            d *= 100.; // m to cm
            cout << d * Constant::CM_UM << ' ';
        }
        for (double& d : _av) 
        {
            d *= 100.; // m to cm
            cout << d / Constant::ANG_CM << ' ';
        }
    }
...
//        }
        ...
        if (a <= _av[0]) // extrapolate propto a
        {
            for (size_t i = 0; i < _lambdav.size(); i++)
                QabsFromFileForA[i] = _Qabsvv[i][0] * a / _av[0];
        }
...
# endif
    ...
    std::vector<double> generateISRF(std::vector<double>& wavelength)
    {
        ...
        double currentG0 = UVdensity / _habing;
        ...
        cout << "Normalized spectrum uv = "<< UVIntensitybis << " (" <<  UVIntensitybis / _habing << " habing)"<< endl;
...
double Photoelectric::ionizationPotential(double a, int Z)
{
    double e2a = _e2 / a;
    double ip_v = (Z + .5) * e2a;
    if (Z >= 0)
...
double Photoelectric::heatingRateAZ(double a, int Z, std::vector<double>& wavelength, std::vector<double>& Qabs, std::vector<double>& isrf)
{
    ...
    for (size_t lambda_index = 0; lambda_index < nLambda; lambda_index++)
    {
        ...
        if (hnu > hnu_pet) 
        {
            ...
            peIntegrandv[lambda_index] = Y * Qabs[lambda_index] * isrf[lambda_index] / hnu * IntE / y2;
        }    
        ...
        if (Z < 0)
        {
            ...
            if (hnu > hnu_pdt)
            {
                ...
                pdIntegrandv[lambda_index] = sigma_pdt * isrf[lambda_index] / hnu * (hnu - hnu_pdt + Emin);
            }
        }
    }  
...
double Photoelectric::heatingRateA(double a, std::vector<double>& wavelength, std::vector<double>& Qabs, std::vector<double>& isrf)
{
    ...
    std::ofstream outvar;
    std::stringstream filename;
    filename << "/Users/drvdputt/Testing/multi-fz/fz_a" << setfill('0') << setw(8) << setprecision(2) << fixed << a / Constant::ANG_CM << ".txt";
    ...
    for (int z = Zmin; z <= Zmax; z++) outvar << z << '\t' << fZ[z - Zmin] << '\n';
    outvar.close();
...
double Photoelectric::energyIntegral(double Elow, double Ehigh, double Emin, double Emax)
{
    double Ediff = Ehigh - Elow;
    double Ediff3 = Ediff * Ediff * Ediff;    
    
    // Compute integral f(E)E dE analytically (it's a parabola)
    double Emax2 = Emax * Emax;
    double Emin2 = Emin * Emin;
    return 6 / Ediff3 * ( -1/4.*(Emax2*Emax2 - Emin2*Emin2) + 1/3.*(Ehigh + Elow)*(Emax2*Emax - Emin2*Emin) - Elow*Ehigh*(Emax2 - Emin2) );
}    

double Photoelectric::yield(double a, int Z, double hnuDiff, double Elow, double Ehigh) 
{
    if (hnuDiff < 0) throw std::range_error("Frequency is smaller than photoelectric threshold.");
    ...
    double y1 = beta2 / alpha2 
                * (alpha2 - 2.*alpha + 2. - 2.*exp(-alpha)) 
                / (beta2 - 2.*beta + 2. - 2.*exp(-beta));
    
    // Calculate y0 from eq 9, 16, 17
    double thetaOverW = hnuDiff; 
    if (Z >= 0) thetaOverW += (Z + 1) * _e2 / a; // eq 9
    thetaOverW /= _W;    
...
int Photoelectric::minimumCharge(double a)
{
    double aA = a / Constant::ANG_CM;
    
    // WD01 eq 23
    double Uait = _carbonaceous ? 3.9 + 0.12 * aA + 2. / aA
                                : 2.5 + 0.07 * aA + 8. / aA; 
    // WD01 eq 24
    return floor( -Uait / 14.4 * aA ) + 1; 
}

double Photoelectric::stickingCoefficient(double a, int Z, int z_i) 
{
    // electrons
    if (z_i == -1)
    {
        ...
        if (Z <= 0)
        {
            if (Z > minimumCharge(a))
            {
                ...
                return 0.5 * (1 - exp(-a / le)) / (1 + exp(20 - NC));            
            }
            else return 0;
        }
        // positive grains
        else return (1 - pElasticScatter) * (1 - exp(-a / le));
    }
    // ions
    else return 1;
}

double Photoelectric::collisionalChargingRate(double a, int Z, int z_i, double m_i)
{
    double stick = stickingCoefficient(a, Z, z_i);
    
    double kT = Constant::BOLTZMAN * _gasTemperature;
    
    double tau = a * kT / _e2 / z_i / z_i; // notes eq 38 (akT / q = akT / e^2 / z^2)
...
double Photoelectric::lambdaTilde(double tau, double ksi)
{
    ...
    {
        return (2. + ksi / tau) * (1. + 1. / sqrt(3. / 2. / tau + 3. * ksi)) * exp(-ksi / (1. + 1. / sqrt(ksi)) / tau);
    }       
    else
    {
...
double Photoelectric::recombinationCoolingRate(double a, const std::vector<double>& fZ, int Zmin)
{
    double kT = Constant::BOLTZMAN * _gasTemperature;
    ...
    particleSum += _electronDensity * sqrt(eightkT3DivPi / _me) * Zsum;
    ...
    double secondTerm = fZ[0] * collisionalChargingRate(a, Zmin, -1, _me) * ionizationPotential(a, Zmin - 1);
                                  ...
                                  int& resultZmax, int& resultZmin, std::vector<double>& resultfZ)
{
    ...
    if (resultZmax < resultZmin) throw std::range_error("Zmax is smaller than Zmin");
    ...
    for (int z = centerZ + 1; z <= resultZmax; z++)
    {
        ...
        double Je = collisionalChargingRate(a, z, -1, _me);
    ...
    for (int z = centerZ - 1; z >= resultZmin; z--)
    {
        ...
        double Je = collisionalChargingRate(a, z + 1, -1, _me);
        int index = z - resultZmin;
    ...
    // Normalize
    double sum = 0.;
    for (double d : resultfZ) sum += d;
    for (double& d : resultfZ) d /= sum;
...
//        Je = collisionalChargingRate(a, z+1, -1, _me);
...
double Photoelectric::yieldFunctionTest()
{
    // Parameters
    const int Z = 10;
    ...
    std::ofstream out("/Users/drvdputt/Testing/yieldTest.dat");
    for(double a : av)
    {
        out << "# a = " << a << '\n';
        const double e2a = _e2 / a;
        ...
        for (size_t n = 0; n < N; n++)
        {
            ...
            if (hnuDiff > 0) out << hnu * Constant::ERG_EV << '\t' << yield(a, Z, hnuDiff, Elow, Ehigh) << '\n';
            hnu += step;
        }
        out << '\n';
    }
...
double Photoelectric::heatingRateTest()
{
    readQabs();
...
#else
    vector<double> wavelength = generateWavelengthGrid();
#endif
    ...
    outRate.open("/Users/drvdputt/Testing/rateTest.txt");
    
    ofstream outAvgQabs;
    outAvgQabs.open("/Users/drvdputt/Testing/avgQabsInterp.txt");
    ...
    for (size_t m = 0; m < Na; m++)
    {
        ...
        for (size_t n = 0; n < Qabs.size(); n++) QabsTimesUlambda[n] = Qabs[n] * isrf[n];

        double uTimesAvgQabs = NumUtils::integrate<double>(wavelength, QabsTimesUlambda);
        double u = NumUtils::integrate<double>(wavelength, isrf);
        ...
        outRate << a / Constant::ANG_CM << '\t' << Photoelectric::heatingRateA(a, wavelength, Qabs, isrf) / totalAbsorbed << '\n';
...
double Photoelectric::chargeBalanceTest()
{
    readQabs();
    ...
    std::ofstream out;
    out.open("/Users/drvdputt/Testing/fZ.txt");
    ...
    for (int z = Zmin; z <= Zmax; z++) out << z << '\t' << fZ[z - Zmin] << '\n';
    out.close();
