#ifndef GASMODULE_GIT_SRC_COLLISIONDATA_H_
#define GASMODULE_GIT_SRC_COLLISIONDATA_H_
...
public:
	...
	/** Use this function to put all the data into the map. Arguments: initial level index,
	    final level index, Array containing q(T) for this transition. Will throw an error if the
	    number of transitions inserted exceeds the size that was reserved at construction. */
	...
	// GETTERS. Pick whichever you find most useful. Seems like a have a little bi-directional
...
private:
	...
	// Also store a list of all the transitions. Impact of this should be pretty minimal.
	std::vector<std::array<int, 2>> _transitionv;
};

#endif /* GASMODULE_GIT_SRC_COLLISIONDATA_H_ */
