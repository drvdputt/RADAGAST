...
/** This class deals with level coefficients. A data class (e.g. H2FromFiles) can inherit from this
    class to re-use the level coefficient infrastructure, if it also wants to read in level
...
protected:
    /** For use by subclass during construction (workaround would be a virtual setup() function.) */
...
public:
    ...
    /** Ouputs some properties about the different line transitions. The results for the number of
        lines, their frequencies [s-1] and their natural widths (decay rate [s-1] / 4 pi) are
        returned by reference. */
    void lineInfo(int& numLines, Array& lineFreqv, Array& naturalLineWidthv) const;

    /** Construct the rate matrix T_ij for the given radiation field and gas properties. This is the
        sum of the spontaneous (Aij), induced (Bij) and collisional (Cij) transitions. The induced
        transitions rates (B coefficients * line power) are derived from the given specific
        intensity. Optionally, the collision coefficients can be returned separately by pointer, so
        they don't have to be calculated again later (the result is exactly the same as calling
        cvv() but it is more efficient to calculate the total matrix and cvv at the same time). Same
        for the induced radiative transition matrix. By passing these pointers, you can also avoid
        having to free the memory for these matrices. [s-1] */
    ...
    /** Calculates the level populations using a simple Boltzman LTE equation. Also serves as an
        example of how to properly set up a LevelSolution object. */
...
private:
    /** Create the matrix [Bij*Pij], where Bij are the Einstein B coefficients (derived from the
        Aij) and Pij is the line power, i.e. the radiation field integrated over the line profile.
        The temperature and collision coefficients are needed, because these influence the shape of
        the line profile. The units of Bij and Pij are often different in the literature and other
        codes (it depends on the units used for the radiation field), but their product should
...
public:
    /** Abstraction of the loop over all lines. Executes thingWithLine for all combinations upper >
        ...
        transitions have line activity, then this function will loop over all elements of the lower
        triangle of the level matrix. */
    ...
    /** Calculates the integrated emission coefficient of a specific line [erg s-1 cm-3 sr-1].
        Multiplying with the line profile [Hz-1] will yield the specific intensity [erg s-1 cm-3
        sr-1 Hz-1]. */
    ...
    /** Return a line profile object that can be used to calculate the (normalized to 1) line
        profile of the "upper-lower" line. The natural line width (the lorenzian contribution) is
        calculated from the total decay rate due to both spontaneous and collisional transitions
        contained in _avv and Cvv, respectively. The thermal line width (the gaussian contribution)
        is calculated from the given temperature and the mass of the particle. */
