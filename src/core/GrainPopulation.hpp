...
public:
    ...
    /** If not a nullptr, then the object can be used to calculate the photoelectric effect (charge
        distribution and heating rate). */
    const GrainPhotoelectricData* photoelectricData() const { return _photoelectricData.get(); }

    /** The total thermal emission of a single grain of size _sizev[m], for a given temperature.
        Uses a lookup table, which is created at construction. [erg s-1] */
