...
/** This class is the backbone of the whole code. It calculates the abundances, level
    ...
    convenience functions are provided in this class to skip having to make a GasSolution object
    (an intermediary GasSolution still appears though). Only using GasState objects drastically
    reduces the amount of includes needed. The GasState objects can be given to a few functions
    in this class to calculate the emissivity and opacity.
