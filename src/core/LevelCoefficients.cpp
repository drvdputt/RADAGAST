...
void LevelCoefficients::setConstants(const EVector& ev, const EVector& gv, const EMatrix& avv, const EMatrix& extraAvv)
{
    _numLv = ev.size();
    _ev = ev;
    _gv = gv;
                                                   ...
                                                   const EMatrix& Cvv) const
{
    ...
    forActiveLinesDo([&](size_t upper, size_t lower) {
        ...
        if (Options::levelcoefficients_reportSpecIntegral)
        {
            ...
            double manualIntegral = TemplatedUtils::integrateFunction<double>(f, specificIntensity.freqMin(),
...
{
    double nu_ij = (_ev(upper) - _ev(lower)) / Constant::PLANCK;
    double constantFactor = Constant::LIGHT * Constant::LIGHT / 8. / Constant::PI / nu_ij / nu_ij * _avv(upper, lower);
