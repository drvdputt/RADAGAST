...
FreeBound::FreeBound()
{
    ...
    if (Options::freebound_debugData)
    {
        ...
        {
            for (size_t iT = 0; iT < _gammaDaggervv.size(1); iT++) out << scientific << _gammaDaggervv(iNu, iT) << '\t';
            out << endl;
        }
        out.close();
        ...
        for (size_t iNu = 0; iNu < _gammaDaggervv.size(0); iNu++)
        {
            for (double logT = 2; logT < 5; logT += 0.01)
            {
			   ...
			   Tleft). */
                double wRight =
                    (logT - _logTemperaturev[iRight - 1]) / (_logTemperaturev[iRight] - _logTemperaturev[iRight - 1]);
