...
namespace GasModule
{
    ...
    /** Abstract class for the non-constant H2 properties, and the functions that depend on them.
        We envision 2 subclasses: a large model which includes a full calculation of the level
        populations, and dissociation / heating / etc. derived from these populations; and a
        smaller model which implements more approximate recipes. */
    class H2Model
    {
    public:
        virtual ~H2Model() = default;

        /** Solve the state of H2, given a constant set of environmental parameters. Depending on
            the sublass, the calculation can be trivial, or extremely heavy (solving the levels).
            The H2 formation tempo [cm-3 s-1] (from grain surfaces only) is need to normalize the
            source term, which describes the formation pumping. */
        ...
        /** TODO: Absorption of kinetic energy by collisional dissociation processes. Since this
            ...
            transferred during the collision, I will need data for this. Have found no candidates
            yet. */
        // dissociationCooling
