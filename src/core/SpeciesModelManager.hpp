...
{
public:
    /** Create an instance of SpeciesModelManager with specific settings for H and H2. Depending
        the given options, different data sets might be loaded, and the behaviour of the make
        functions will be influenced. The options are undocumented for now, as the are subject to
        change. Look at the 'if's in the implementation. */
    ...
    /** Create a new H2Model polymorphically. Depending on the heuristics given, either a
        BigH2Model or a SmallH2Model might be created. A BigH2Model will get access to the H2 data
        stored here, while a SmallH2Model ideally does not need it. */
