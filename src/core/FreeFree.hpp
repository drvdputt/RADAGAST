...
{
public:
    /** All the data is read in during construction. The given frequency grid will be used for the
        output. */
    FreeFree();
...
public:
    ...
    /** Calculate the opacity coefficient for the free-free continuum for a specific frequency and
        temperature. The units of the returned coefficient is [cm-7]: opacity [cm-1] per density
        squared [cm-6]. Multiplying with ne*np will give the opacity in [cm-1]. */
    double opacityCoefficient(double nu, double T) const;

    /** Calculate the heating due to direct absorption of photons the free electrons moving in the
        field of the free protons. */
    ...
    /** Calculate the cooling due to Bremsstrahlung, given the product of the electron and proton
        densities, and their kinetic temperature. [erg cm-3 s-1]. */
...
private:
    ...
    /* The ((base 10!) log of the) minima and maxima defining the u - gamma^2 grid, as well as the
    ...
    /* The natural log of the data read from the file containing the integrated Gaunt factor,
       ...
       gamma^2 direction. I'm not really sure why we store the gamma^2 values here. Right now I'm
       just going to adapt the implementation from interpolate4.c */
