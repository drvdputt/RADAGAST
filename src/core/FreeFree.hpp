...
public:
	/** Calculate the emission coefficient for the free-free continuum for all frequencies.
	    The units are [density^-1][power]/[frequency interval] cm^3 erg / s / cm. The
...
private:
	...
	/* The ((base 10!) log of the) minima and maxima defining the u - gamma^2 grid, as well
	   as the step size in log space. */
	double _loggamma2Min{0}, _loguMin{0}, _logStep{0}, _loggamma2Max{0}, _loguMax{0};

	/* The natural log of the data read from the file containing the integrated Gaunt
	   factor, indexed on (gamma^2). Note that there is a different number of data points
	   here in the gamma^2 direction. I'm not really sure why we store the gamma^2 values
	   here. Right now I'm just going to adapt the implementation from interpolate4.c */
