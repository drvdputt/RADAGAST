...
double GrainH2Formation::surfaceH2FormationRateCoeff(const Array& sizev,
                                                     ...
                                                     const Array& densityv, double Tgas) const
{
	...
	const Array& coeffPerGrainPerHPerSizev =
	                surfaceH2FormationRateCoeffPerSize(sizev, temperaturev, Tgas);
	double total = (densityv * coeffPerGrainPerHPerSizev).sum();
	return total;
}

Array GrainH2Formation::surfaceH2FormationRateCoeffPerSize(const Array& sizev,
                                                           const Array& temperaturev,
                                                           double Tgas) const
{
	...
	{
		double Td{temperaturev[i]}; // Cross section of the grain. This needs to be an integrated quantity over the
		...
		if (std::isfinite(product))
			formationPerGrainPerHPerSizev[i] = product;
		// Else it will stay 0
	}
	return formationPerGrainPerHPerSizev;
}

Array GrainH2Formation::surfaceH2FormationHeatPerSize(const Array& sizev,
                                                      const Array& temperaturev, double Tgas,
                                                      double nH) const
{
	const Array& coeffPerGrainPerHPerSizev =
	                surfaceH2FormationRateCoeffPerSize(sizev, temperaturev, Tgas);
