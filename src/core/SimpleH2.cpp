...
double SimpleH2::orthoPara() const
{
    return _ortho;
}
...
Array SimpleH2::opacityv(const Array& oFrequencyv) const
{
...
double SimpleH2::gloverAbel08Cooling(const CollisionParameters& cp) const
{
    ...
    // equation 32
    double coolH2 = _ortho * _ortho * GloverAbel08::coolOrthoOrtho(T) + _para * _ortho * GloverAbel08::coolParaOrtho(T)
