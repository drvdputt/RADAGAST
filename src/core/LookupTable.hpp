...
public:
	...
	/** Create a lookup table for a set of functions y_i(x). First argument: x-values.
	    Second argument: Table containing y_i(xv) on each row (dimension should be
	    (numFunctions, numX). */
	...
	/** From data file in repo. Needs to be simple column file (see SimpleColumnFile) with
	    the data in the first two columns. The resulting lookup table will have one y_i per
	    column (depends on numCols given).*/
