...
namespace RADAGAST
{
    double WD01::atomMass(bool carbonaceous)
    {
        ...
        // silicates from silicate_0m010.opc in cloudy (atomic mass of Si is 28, but theres also
        // some O, Mg and Fe mixed in).
    ...
    double WD01::workFunction(bool carbonaceous) { return carbonaceous ? 4.4 * Constant::EV : 8 * Constant::EV; }
    ...
    double WD01::eMin(double a, int Z)
    {
        ...
        {
            // replace by van Hoof (2004) eq 1 (or WD06 eq 3). Note that the factor e^2 / a has
            // gone missing in WD06 eq 3.
            if (Z >= -1)
            {
                return 0;
            }
    ...
    double WD01::energyIntegral(double Elow, double Ehigh, double Emin)
    {
        ...
        // integration range is [Emin, Emax = Ehigh]. Emin has to be given, because we do not want
        // to integrate over electrons that do not escape.
        ...
        // is a fourth order polynomial: a/4 (max4 - min4) + b/3 (max3 - min3) + c/2 (max2 - min2).
    ...
    double WD01::yield_cached(double a, int Z, double hnuDiff, double Emin, bool carbonaceous, double y1_cached)
    {
        ...
        // Energy distribution parameters (parabola that becomes zero at Elow and Ehigh. See WD01
        // text between eq 10 and 11).
        double Elow, Ehigh;
        if (Z < 0)
        {
            Elow = Emin;
    ...
    {
        // Calculate y1 from grain properties and eq 13, 14 double imaginaryRefIndex = 1; // should
        // be wavelength-dependent double la = Constant::LIGHT / nu / 4 / Constant::PI /
        // imaginaryRefIndex;

        // value from 1994-Bakes. WD01 uses the one in the comment above, which is more annoying to
        // calculate.
