...
{
    if (_frequencyv.size() == 0 || _valuev.size() == 0)
        Error::runtime("Zero size Array passed to Spectrum");
...
double Spectrum::evaluate(double frequency) const
{
    ...
    {
        double val = TemplatedUtils::evaluateLinInterpf(frequency, _frequencyv, _valuev);
        return val;
    }
}
...
Array Spectrum::binned(Array frequencyv) const
{
    ...
    for (size_t i = 0; i < frequencyv.size(); i++)
    {
        ...
        double nuMin = i == 0 ? frequencyv[0] : (frequencyv[i] + frequencyv[i - 1]) / 2.;
        // For the last bin, set the right boundary to freq[n-1] instead of a halfway
        // point
        double nuMax =
            i == frequencyv.size() - 1 ? frequencyv[frequencyv.size() - 1] : (frequencyv[i + 1] + frequencyv[i]) / 2.;

        // Outside of the data range, this average will be 0 of course
        if (nuMax < _freqMin || nuMin > _freqMax) continue;
