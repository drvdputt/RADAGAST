...
namespace
{
    ...
    Array generateQabsv(const QabsDataSet& qds, double a, const Array& frequencyv)
    {
        ...
        for (size_t w = 0; w < wavelengthv.size(); w++)
        {
            ...
            QabsWav[w] =
                TemplatedUtils::interpolateRectangular(wav, a, wLeft, wRight, aLow, aUp, q[wIndex - 1][aIndex - 1],
                                                       q[wIndex][aIndex - 1], q[wIndex - 1][aIndex], q[wIndex][aIndex]);
        }
...
vector<double> Testing::freqToWavGrid(const vector<double>& frequencyv)
{
    ...
    for (auto rit = frequencyv.rbegin(); rit != frequencyv.rend(); rit++) wavelengthv.push_back(Constant::LIGHT / *rit);
    return wavelengthv;
}
...
void Testing::plotIonizationStuff()
{
    ...
    for (double freq = 0; freq < 6.e16; freq += 1e14)
    {
        ...
        if (freq > tr)
            ...
            sigmaTheoretical = 0;
        out << freq << "\t" << sigma << "\t" << sigmaTheoretical << "\t" << (sigma - sigmaTheoretical) / sigma << endl;
    }
    out.close();
...
void Testing::writeGasState(const string& outputPath, const GasModule::GasInterface& gi, const GasModule::GasState& gs)
{
    ...
    auto evaluateSpectrum = [&](double f) { return TemplatedUtils::evaluateLinInterpf<double>(f, eFrequencyv, emv); };
