...
public:
	...
	/** Recalculate the temperature for each size, by finding the temperature for which <
	    ...
	    slow. No idea how to handle this if stochastically heated grains would be a thing.
	...
	/** Recalculate the charge distribution for each size using the
	    ...
	    grain photoelectric data), the default charge distribution is kept (all grain
	    charges are at 0). */
	void
	...
	/** Calculate the total energy transfer to the gas due to the thermalization of
	    photoelectrons ejected from the grains. Should be called after the charge
	    distributions have been calculated (for the same env). */
	double
	...
	/** Calculate the total energy transfer from the gas to the grains due to collisions.
	    Should be called after the charge distributions have been calculated (for the same
	    env). */
	double
	collisionalGasCooling(const GrainPhotoelectricCalculator::Environment& env) const;

	/** Calculate the total H2 formation rate per H density unit [s-1], summed over all
	    sizes. Should be called after the temperatures grain have been updated. */
