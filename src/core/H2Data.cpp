...
namespace GasModule
{
    ...
    void H2Data::readTransProbs()
    {
        ...
        // Radiative transitions within ground stated. Fills _avv with data from Wolniewicz (1998)
    ...
    void H2Data::readDirectDissociation()
    {
        ...
        while (getline(cont_diss, line))
        {
            ...
            {
                // Get the three lines starting with "#!". I don't think I need the 2nd and 3rd
                // lines though.
                string line1, line2;
                ...
                // When this loop exits, we should be at the start of the next comment block, or at
                // the end of the file.
    ...
    void H2Data::readCollisionFile(const string& repoFile, CollisionPartner iPartner)
    {
        ...
        DEBUG("Read in " << _qdataPerPartner[iPartner].transitionv().size() << " collision coefficients from "
                         << repoFile << '\n');
    }
        ...
        for (int p = 0; p < _numPartners; p++)
        {
            for (int i = 0; i < _startOfExcitedIndices; i++)
            {
                for (int j = i + 1; j < _startOfExcitedIndices; j++)
                {
                    ...
                    // Only fill in downward rates. Upward rates will depend on temperature using
                    // otherDirectionC.
                    if (sigma > 0)  // e_i > e_j
    ...
    void H2Data::addGBarCvv(EMatrix& the_cvv, double kT, CollisionPartner iPartner, double nPartner) const
    {
        for (int i = 0; i < _startOfExcitedIndices; i++)
        {
            for (int j = i + 1; j < _startOfExcitedIndices; j++)
            {
                ...
                // Determine which is upper and which is lower (I don't know of j > i guarantees
                // that e_j > e_i)
                int u, l;
