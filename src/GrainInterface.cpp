...
GrainInterface::Population::Population(const Array& sizev, const Array& densityv,
                ...
                : _sizev{sizev}, _densityv{densityv}, _temperaturev{temperaturev}, _qAbsvv{qAbsvv},
                  _type(GrainTypeFactory::makeCustom(sfcInteractionPar, heatingAvailable,
                                                     workFunction, ionizationPotentialf,
                                                     photoElectricYieldf, autoIonizationThresholdf,
                                                     stickingCoefficientf))
{
	Error::equalCheck("sizev.size() and densityv.size()", sizev.size(), densityv.size());
	Error::equalCheck("sizev.size() and temperaturev.size()", sizev.size(),
	                  temperaturev.size());
