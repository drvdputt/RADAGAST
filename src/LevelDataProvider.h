...
/* When an object of the class NLevel is set up, it needs some constant data, and some data that can
   vary with temperature. This data often needs to be derived from other data that was read in from
   files. To abstract the read-in of files, and the conversion of their data to a usable format for
   the calculations, this class has been created. Subclasses of LevelDataProvider should read in a
   set of data, or have some data hardcoded, and provide implementations of the functions
...
public:
	...
	/* Returns a matrix containing the collisional transition rates (already multiplied with the
	   partner density), for a given temperature and proton and electron densities. Calculate
	   the collision rates Cij. Rij = Cij * ni = q_ij * np * ni --> Cij = q_ij * np with np the
	   density of collision partners */
