...
public:
	...
	/* Makes a plot of the heating efficiency in function of the grain size. Saved as a two-column file
...
private:
	...
	// Gas properties
	const double _hydrogenDensity
	{ 2.5e1 };
	const double _ionizationFraction
	{ 3.e-4 };
	const double _electronDensity
	{ _hydrogenDensity * _ionizationFraction };
	double _gasTemperature
	{ 1.e3 };
	...
	{ _carbonaceous ? 4.4 / Constant::ERG_EV : 8 / Constant::ERG_EV };
	...
	const size_t _nWav
	{ 200 };
	const double _minWav
	{ 0.0912 * Constant::UM_CM }; // cutoff at 13.6 eV
	const double _maxWav
	{ 1000 * Constant::UM_CM };
