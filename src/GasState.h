...
// Private constructor, only to be used by friended class which acts as a factory and can fill in all the members
	GasState(const std::vector<double>& frequencyv, const std::vector<double>& previousISRFv,
			const std::vector<double>& emissivityv, const std::vector<double>& opacityv,
			const std::vector<double>& scatteringOpacityv, double T, double f) :
			_frequencyv(frequencyv), _previousISRFv(previousISRFv), _emissivityv(emissivityv), _opacityv(
					opacityv), _scatteringOpacityv(scatteringOpacityv), _T(T), _ionizedFraction(
					f)
	{
	}
...
// Memory-heavy, but simple implementation: just store all the output
	std::vector<double> _frequencyv, _previousISRFv, _emissivityv, _opacityv, _scatteringOpacityv;
